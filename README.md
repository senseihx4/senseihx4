# Hi, I'm Kartik Sharma 👋

BCA graduate and Python Full Stack Developer intern, building real-world projects across web, APIs, and AI/ML. Every project on this profile was built from scratch — no tutorials, no shortcuts.

Currently interning as a Python Full Stack Developer and diving deep into AI/ML. Actively looking for SDE roles at product-driven companies.

---

## 🛠 Tech Stack

**Languages**
![Python](https://img.shields.io/badge/Python-3776AB?style=flat&logo=python&logoColor=white)
![JavaScript](https://img.shields.io/badge/JavaScript-F7DF1E?style=flat&logo=javascript&logoColor=black)
![HTML/CSS](https://img.shields.io/badge/HTML%2FCSS-E34F26?style=flat&logo=html5&logoColor=white)

**Frameworks & Tools**
![Flask](https://img.shields.io/badge/Flask-000000?style=flat&logo=flask&logoColor=white)
![Django](https://img.shields.io/badge/Django-092E20?style=flat&logo=django&logoColor=white)
![REST API](https://img.shields.io/badge/REST%20APIs-005571?style=flat)
![SQL](https://img.shields.io/badge/SQL-4479A1?style=flat&logo=mysql&logoColor=white)

**Exploring**
![AI/ML](https://img.shields.io/badge/AI%2FML-FF6F00?style=flat&logo=tensorflow&logoColor=white)

---

## 🚀 Featured Projects

| Project | Description | Tech |
|---|---|---|
| [Netflix Clone](https://github.com/senseihx4/Netflix-clone-) | Front-end clone of Netflix UI | HTML, CSS |
| [AnyDesk Clone](https://github.com/senseihx4/anydesk-clone) | Remote desktop app clone | Python |
| [Sports Court Booking](https://github.com/senseihx4/-sports-court-booking) | Court booking system | Python |
| [Student Result API](https://github.com/senseihx4/student-result-api-with-pdf-send-to-email) | API that generates result PDFs and emails them | Python |

---

## 📈 GitHub Stats

![Kartik's GitHub Stats](https://github-readme-stats.vercel.app/api?username=senseihx4&show_icons=true&hide_border=true&count_private=true)
![Top Languages](https://github-readme-stats.vercel.app/api/top-langs/?username=senseihx4&layout=compact&hide_border=true&langs_count=6)
---

## 📬 Connect with Me

[![LinkedIn](https://img.shields.io/badge/LinkedIn-0A66C2?style=flat&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/kartik-sharma-023a85322/)
[![Email](https://img.shields.io/badge/Email-D14836?style=flat&logo=gmail&logoColor=white)](mailto:YOUR-EMAIL-HERE)

---

*Open to internships, full-time SDE roles, and open source collaboration.*
